...
<!-- ms-sm-auto col-lg-10 px-md-4 -->
     <div class="main-content" id="mainContent">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center mb-2 border-bottom">
        <h1 class="h2 " style="margin-left: 15%">Supplier Deliver List </h1>
          ...
          <div class="card-body">
            <table id="mytable" class="table table-condensed table-bordered table-hover">
              ...
              <tbody>
                ...
                $ptotal = 0;
                $i = 1;
                $fees = $conn->query("SELECT d.*,s.* FROM supplierdeliver d inner join supplier s on d.name = s.supplier_id ");
                ...
                <tr>
                  ...
                  <td class="text-center">
                    <button class="btn btn-sm btn-outline-primary view_exam" type="button" data-id="<?php echo $row['sid'] ?>"
                      data-examID="<?php echo $row['sid'] ?>">View</button>


                    <!-- <button class="btn btn-sm btn-outline-primary edit_exam" type="button" data-id="<?php echo $row['sid'] ?>" >Edit</button> -->
                    <button class="btn btn-sm btn-outline-danger supplier_company" type="button" data-id="<?php echo $row['sid'] ?>">Delete</button>
                  </td>
                </tr>
                <?php endwhile; ?>
              </tbody>
...
$('.view_exam').click(function(){
    uni_modal("Report Details","view_supplierlist.php?id="+$(this).attr('data-examin_ID')+"&id="+$(this).attr('data-id'),"mid-large")
    
  })

  
  $('#SupplierDeliverlist').click(function(){
    uni_modal("Suppliers Entry","SupplierDeliverlistcontrolimage.php",'small')
    
  })

  $('.edit_exam').click(function(){
    uni_modal("Edit Suppliers ","SupplierDeliverlistcontrol.php?id="+$(this).attr('data-id'),'large')
    
  })
  $('.supplier_company').click(function(){
    _conf("Are you sure to delete this Suppliers?","supplier_company",[$(this).attr('data-id')])
  })
