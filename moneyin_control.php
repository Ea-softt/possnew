...
if(isset($_GET['id'])){
$qry = $conn->query("SELECT mi.*,md.description,md.money_id,mi.name as Name  FROM moneyin mi inner join moneyin_des md on mi.id = md.money_id  where id= ".$_GET['id']);
...
<div class="container-fluid">
    <form action="" id="manage-moneyin">
        <input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
        <div class="row">
        <div class="col-md-3 border-right ">
            <h5><b>Money In Details</b></h5>
            <hr>
           ...
           <div class="mb-3">
             ...
             <?php
               for($i = 0; $i <= 31; ++$i){
                ...
                $days=date('d',$time);
                printf('<option class="text-center" >%s</option>',$day_valye,$days);
                //printf('<input type="text" class="text-center" value="$day_valye">',$day_valye,$days);
               }
               ...
               <option selected="" value="<?php echo isset ($year)? $year :'' ?>"><?php echo isset ($year)? $year :'' ?></option>
                ...
                <div class="form-group">
                 ...
                 <div class="form-group pt-1">
                    ...
                    <button class="btn btn-primary btn-sm" type="button" id="add_moneyin">Add to List</button>
                </div>
           </div>
           ...
           <div class="col-lg-6">
            <table class="table table-condensed" id="amount-list">
                ...
                <tbody>
                    <?php
                        if(isset($id)):
                        $fees = $conn->query("SELECT * FROM moneyin_des where  money_id= $id");
                        $total = 0;
...
<div id="moneyin_clone" style="display: none">
     <table >
            <tr>
                <td class="text-center"><button class="btn-sm btn-outline-danger" type="button" onclick="rem_list($(this))" ><i class="fa fa-times"></i></button></td>
                <td>
...
<script>
    $('#manage-moneyin').on('reset',function(){
        $('#msg').html('')
        $('input:hidden').val('')
    })
    ...
    function rem_list(_this){
     ...
     $(document).on('submit', '#manage-moneyin', function(e) {
        ...
        $.ajax({
            url:'easoftfun.php?action=save_moneyin',
