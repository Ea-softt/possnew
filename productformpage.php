...
<head>
    ...
    <!-- Accounting JS (Required for formatMoney) -->
...
<body>
    ...
    <div class="main-content" id="mainContent">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center mb-2 border-bottom bg-success text-white p-2 rounded">
            <h1 class="h4 m-0">From Warehouse to Stock</h1>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-7 border-end">
                    ...
                    <div id="content" class="mr-15">
                        <div id="price_column" class="table-responsive table-wrapper-scroll-y my-custom-scrollbar-y">
                            <table class="table table-striped table-bordered table-hover w-100" style="cursor: pointer;">
                ...
                <div class="col-lg-5"> 
                    ...
                    <div id="product_area" class="table-responsive table-wrapper-scroll-y my-custom-scrollbar-a">
                        <table id="mytable" class="table1 table-striped w-100 font-weight-bold tablet table-hover" style="cursor: pointer;">
                            <thead class="bg-dark text-white">
                                <tr class='text-center'>
    ...
    <script>
            ...
            if(name){
                $.ajax({
                    ...
                    success: function (Response){
                        $('#products_list').html(Response); // Assuming dropdownstock.php returns rows
                    }
                });
            }
        };
        ...
        $(document).on('click', '#submit', function(){
            ...
            var images = $('#images').val(); // Note: #images input is commented out in HTML
            var supplierid =[];
            
            $('.sid').each(function(){ sid.push($(this).text()); });
            $('.supplierid').each(function(){ supplierid.push($(this).text()); });
            $('.product').each(function(){ product.push($(this).text()); });
            $('.barcode').each(function(){ barcode.push($(this).text()); });
            $('.sprice').each(function(){ sprice.push($(this).text().replace(/,/g, "").replace("Ghc","")); });     
            $('.cprice').each(function(){ cprice.push($(this).text().replace(/,/g, "").replace("Ghc","")); });
            $('.unit1').each(function(){ unit1.push($(this).text()); });
            $('.quantity1').each(function(){ quantity1.push($(this).text()); });
                    ...
                    $.ajax({
                        ...
                        success: function(data){
                            if(data.trim() == "Successfully"){  
                                ...
                                // Handle failure or other response
                                alert_toast("Saved with response: " + data, 'info');
