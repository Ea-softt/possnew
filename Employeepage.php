     ...
     input[type=checkbox]
{
  ...
  cursor:pointer;
        ...
        :root {
            --bg-color: #f8f9fa;
            --sidebar-bg: #212529;
            --sidebar-text: rgba(255, 255, 255, 0.8);g
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="card">                  
                    ...
                    <a class="btn btn-primary btn-sm" href="javascript:void(0)" id="employee_control">
                    ...
                    <div class="card-body">
                        <table id="mytable" class="table table-condensed table-bordered table-hover">
                            ...
                            <tbody>
                                <?php 
                                $i = 1;
                                $student = $conn->query("SELECT * FROM newemployee  order by EmpID asc ");
                                while($row=$student->fetch(PDO::FETCH_ASSOC)):
                                ?>
                                <tr>
                                    <td class="text-center"><?php echo $i++ ?></td>
                                    <td class="text-center">
                                        <p><?php echo $row['EmpID']. " - " . $row['FullName'] ?></p>
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td class="text-center">                    
                                        <button class="btn btn-sm btn-outline-primary view_newEmployee" type="button" data-id="<?php echo $row['EmpID'] ?>"
                                    data-employee="<?php echo $row['EmpID'] ?>">View</button>


                                    <button class="btn btn-sm btn-outline-primary edit_newemployee" type="button" data-id="<?php echo $row['EmpID'] ?>" >Edit</button>
                                        <button class="btn btn-sm btn-outline-danger delete_newemployee" type="button" data-id="<?php echo $row['EmpID'] ?>" >Delete</button>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
...
<!-- <script src="bootstrap5/bootstrap.bundle.min.js"></script>
   ...
   <script>
    ...
    $('#employee_control').click(function(){
        uni_modal("New Employee's Form","employee_control.php",'large')
        
    })

    $('.view_newEmployee').click(function(){
    uni_modal("Report Details","view_newEmployee.php?EmpID="+$(this).attr('data-employee')+"&cid="+$(this).attr('data-id'),"mid-large")
    
  })


    $('.edit_newemployee').click(function(){
        uni_modal("Manage Course and Fees Entry","employee_control.php?EmpID="+$(this).attr('data-id'),'large')
        
    })
    $('.delete_newemployee').click(function(){
        _conf("Are you sure to delete this course?","delete_newemployee",[$(this).attr('data-id')])
    })
    
    
    function delete_newemployee($EmpID){
        start_load()
        $.ajax({
            url:'easoftfun.php?action=delete_newemployee',
            method:'POST',
            data:{EmpID:$EmpID},
            success:function(resp){
                if(resp==1){
                    alert_toast("Selected Employee successfully deleted",'success')
                    setTimeout(function(){
                        location.reload()
                    },1000)
